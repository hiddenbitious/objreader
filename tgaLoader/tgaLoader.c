...
int LoadCompressedTGA(Texture * texture, const char *filename, FILE * fTGA)		// Load COMPRESSED TGAs
{
	...
	do
	{
			...
			for(short counter = 0; counter < chunkheader; counter++)		// Read RAW color values
			{
				if(fread(colorbuffer, 1, tga.bytesPerPixel, fTGA) != tga.bytesPerPixel) // Try to read 1 pixel
				{
					...
					return 0;														// Return failed
																						...
																						// write to memory
				texture->imageData[currentbyte		] = colorbuffer[0];				    // Flip R and B vcolor values around in the process
		...
		else																			// chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
		{
			...
			{																			// by the header
				texture->imageData[currentbyte		] = colorbuffer[0];					// switch R and B bytes areound while copying
				texture->imageData[currentbyte + 1	] = colorbuffer[1];
				texture->imageData[currentbyte + 2	] = colorbuffer[2];
...
Texture *
loadGLTexture(const char *filename)
{
